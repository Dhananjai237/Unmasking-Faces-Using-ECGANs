...
class DataGenerator:
    ...
    def get_dataset_examples(self, n=10, test_dataset=False):
        """
        Returns `n` images from the dataset. If `test_dataset` parameter
        is not provided or False, it will return images from the training part of the dataset.
        If `test_dataset` parameter is True, it will return images from the testing part of the dataset.
        """
        if test_dataset:
            data_path = self.test_data_path
        else:
        ...
        outputs_folder = os.path.join(data_path, 'outputs')

        input_files = sorted(os.listdir(inputs_folder))[:n]
        output_files = sorted(os.listdir(outputs_folder))[:n]

        inputs = [os.path.join(inputs_folder, img) for img in input_files]
        outputs = [os.path.join(outputs_folder, img) for img in output_files]
